...
with open(os.path.join(BASE_DIR, "products.json"), "r") as products_file:
    products = json.load(products_file)


# Holds the order id and a dictionary of the order and reason for rejection
rejected_orders = {}
...
# Validate orders
with open(os.path.join(BASE_DIR, "orders.json"), "r") as orders_file:
    orders = json.load(orders_file)
    validate(orders, products, rejected_orders, seen_id)

invalid_order_ids = rejected_orders.keys()
valid_orders = []
for order in orders:
    if order["id"] not in invalid_order_ids:
        valid_orders.append(order)
        
with open("rejection_list.json","w") as rejection_list:
    rejections = json.dumps(rejected_orders, indent = 4)
    rejection_list.write(rejections)
        
# File containing valid orders
with open("validated_orders.json","w") as validated_orders_file:
    valid_orders_str = json.dumps(valid_orders, indent = 4)
    validated_orders_file.write(valid_orders_str)
